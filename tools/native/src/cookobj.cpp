...
void parse_tileset(const char* tsx_path, int firstgid, ObjectMap& map) {
    ...
    int tilecount = atoi(tileset->first_attribute("tilecount") ? tileset->first_attribute("tilecount")->value() : "0");
    std::cerr << "DEBUG parse_tileset: " << tsx_path << std::endl;
    std::cerr << "  firstgid=" << firstgid << ", tilecount=" << tilecount << std::endl;
    ...
    // Pre-load TOML if exists
    FILE* toml_f = fopen(toml_path, "r");
    toml_table_t* toml_root = nullptr;
    if (toml_f) {
        char errbuf[256];
        toml_root = toml_parse_file(toml_f, errbuf, sizeof(errbuf));
        fclose(toml_f);
    }
    ...
    for (rapidxml::xml_node<>* tile = tileset->first_node("tile"); tile; tile = tile->next_sibling("tile")) {
        ...
        std::cerr << "  Processing tile id=" << tid << ", gid=" << gid << ", type=" << type;
        ...
        // Check TOML for dummy flag
        if (toml_root) {
            toml_table_t* obj_tab = toml_table_in(toml_root, type);
            if (obj_tab) {
                toml_datum_t dummy_val = toml_bool_in(obj_tab, "dummy");
                if (dummy_val.ok && dummy_val.u.b) {
                    map.obj_mapping[gid] = DUMMY_RING_3H;
                    std::cerr << " -> DUMMY (from TOML)" << std::endl;
                    continue;
                }
            }
        }
        ...
        if (obj_type < 0 && !map.is_level_specific) {
            std::cerr << " -> SKIPPED (unknown common type)" << std::endl;
            continue;
        }
        ...
        map.objects[gid] = obj;
        int mapped_val = map.is_level_specific ? (gid - firstgid) : obj_type;
        map.obj_mapping[gid] = mapped_val;
        std::cerr << " -> ADDED (obj_id=" << obj.id << ", mapped=" << mapped_val << ")" << std::endl;
    }
    
    if (toml_root) toml_free(toml_root);
    
    map.num_objs = map.objects.size();
    std::cerr << "  Total in obj_mapping: " << map.obj_mapping.size() << ", objects: " << map.objects.size() << std::endl;
    delete[] cstr;
}

void parse_tmx(const char* tmx_path, std::map<std::string, ObjectMap>& maps, std::vector<Placement>& placements) {
    std::cerr << "DEBUG parse_tmx: Received " << maps.size() << " tilesets" << std::endl;
    for (auto& [name, map] : maps) {
        std::cerr << "  Tileset '" << name << "': " << map.obj_mapping.size() << " entries" << std::endl;
        // Check for GID 161 specifically
        auto it161 = map.obj_mapping.find(161);
        if (it161 != map.obj_mapping.end()) {
            std::cerr << "    GID 161 EXISTS -> " << it161->second << std::endl;
        } else {
            std::cerr << "    GID 161 MISSING" << std::endl;
        }
        // Print first few
        // Print first few entries
        int count = 0;
        for (auto& [gid, val] : map.obj_mapping) {
            if (count++ < 5) {
                std::cerr << "    GID " << gid << " -> " << val << std::endl;
            }
        }
    }
    ...
    for (rapidxml::xml_node<>* og = objects_group->first_node("objectgroup"); og; og = og->next_sibling("objectgroup")) {
        for (rapidxml::xml_node<>* obj = og->first_node("object"); obj; obj = obj->next_sibling("object")) {
            ...
            p.parent_id = 0;
            // Python adds +32 to center X position
            p.x = atoi(obj->first_attribute("x")->value()) + 32;
